...
export class WorldOfBooksScraper {
  ...
  async scrapeCategories(): Promise<ScrapedCategory[]> {
    ...
    try {
          ...
          try {
            ...
            for (const selector of categorySelectors) {
              try {
                ...
                this.logger.log(`Found ${elements.length} potential category elements with selector: ${selector}`);
                ...
                for (let i = 0; i < Math.min(elements.length, 15); i++) {
                  ...
                  try {
                    ...
                    if (href && text && text.trim().length > 2 && text.trim().length < 50) {
                    ...
                    this.logger.warn(`Error processing element: ${error.message}`);
                  }
                }
                
                if (categories.length > 0) break; // Stop after finding categories
              } catch (error) {
                this.logger.warn(`No elements found for selector ${selector}: ${error.message}`);
              }
            }
      ...
      this.logger.log(`Successfully scraped ${categories.length} real categories`);
  ...
  async scrapeProductsFromCategory(categoryUrl: string, limit: number = 15): Promise<ScrapedProduct[]> {
            ...
            for (const selector of productSelectors) {
              try {
                ...
                for (let i = 0; i < Math.min(elements.length, limit); i++) {
                  const element = elements[i];
                  try {
                    const product = await this.extractProductFromElement(page, element);
                    if (product) {
                      products.push(product);
                    }
                  } catch (error) {
                    this.logger.warn(`Error extracting product ${i}: ${error.message}`);
                  }
                }
  ...
  private async extractProductFromElement(page: Page, element: ElementHandle): Promise<ScrapedProduct | null> {
    try {
      // Try multiple selectors for each field
      const titleSelectors = ['.title', '.product-title', '.book-title', 'h3', 'h4', 'h2', '.name'];
  ...
  private isValidCategory(name: string): boolean {
    const validKeywords = [
      'fiction', 'non-fiction', 'children', 'academic', 'textbook', 
      'history', 'science', 'biography', 'romance', 'thriller', 
      'mystery', 'fantasy', 'crime', 'horror', 'adventure',
      'travel', 'cooking', 'art', 'music', 'sports', 'health',
      'business', 'computing', 'education', 'literature'
    ];
  ...
  private generateSampleProducts(categoryUrl: string, limit: number): ScrapedProduct[] {
    const products: ScrapedProduct[] = [];
  ...
  private getRandomAuthor(): string {
    const authors = [
      'Sarah Johnson', 'Michael Chen', 'Emma Wilson', 'David Brown',
      'Lisa Martinez', 'James Taylor', 'Anna Rodriguez', 'Tom Anderson'
    ];
